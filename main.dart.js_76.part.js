((a,b,c)=>{a[b]=a[b]||{}
a[b][c]=a[b][c]||[]
a[b][c].push({p:"main.dart.js_76",e:"beginPart"})})(self,"$__dart_deferred_initializers__","eventLog")
$__dart_deferred_initializers__.current=function(a,b,c,$){var J,A,B,D,C={
fg(d,e,f){return C.eZ9(d,e,f)},
eZ9(d,e,a0){var x=0,w=A.k(y.j),v,u=2,t,s,r,q,p,o,n,m,l,k,j,i,h,g,f
var $async$fg=A.l(function(a1,a2){if(a1===1){t=a2
x=u}while(true)switch(x){case 0:u=4
k=y.g
s=A.a8(["Content-Type","application/json"],k,k)
k=$.ah
j=a0==null?0:a0
i=e==null?0:e
h=d==null?0:d
r=A.aO("GET",A.ad(k+"/Coding/GetSectors?townId="+j+"&regionId="+i+"&areaId="+h,0,null))
r.r.F(0,s)
x=7
return A.d(J.aQ(r),$async$fg)
case 7:q=a2
x=8
return A.d(B.E.am(q.w),$async$fg)
case 8:p=a2
if(q.b===200){o=B.k.C(0,p,null)
n=A.b([],y.o)
for(k=J.b0(J.A(o,"data"));k.O();){m=k.ga4(k)
J.cf(n,D.bG6(m))}v=new A.x(n,1,null,null,y.j)
x=1
break}else{k=A.X(B.k.C(0,p,null),y.b)
v=new A.x(null,0,k.d,null,y.j)
x=1
break}u=2
x=6
break
case 4:u=3
f=t
l=A.V(f)
k=J.u(l)
v=new A.x(null,0,k,null,y.j)
x=1
break
x=6
break
case 3:x=2
break
case 6:case 1:return A.i(v,w)
case 2:return A.h(t,w)}})
return A.j($async$fg,w)}}
J=c[1]
A=c[0]
B=c[2]
D=c[335]
C=a.updateHolder(c[334],C)
var z=a.updateTypes([])
var y={j:A.I("x<q<aG>>"),o:A.I("n<aG>"),g:A.I("e"),b:A.I("@")}};
((a,b)=>{a[b]=a.current
a.eventLog.push({p:"main.dart.js_76",e:"endPart",h:b})})($__dart_deferred_initializers__,"ar9q0z7EVd6RL2x1DOo6R0ruues=");