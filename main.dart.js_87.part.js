((a,b,c)=>{a[b]=a[b]||{}
a[b][c]=a[b][c]||[]
a[b][c].push({p:"main.dart.js_87",e:"beginPart"})})(self,"$__dart_deferred_initializers__","eventLog")
$__dart_deferred_initializers__.current=function(a,b,c,$){var J,A,B,C={
m0(){var x=0,w=A.i(y.m),v,u=2,t,s,r,q,p,o,n,m,l,k,j,i
var $async$m0=A.j(function(d,e){if(d===1){t=e
x=u}while(true)switch(x){case 0:u=4
k=y.g
s=A.a8(["Content-Type","application/json"],k,k)
r=A.aN("GET",A.ad($.ah+"/Coding/GetCities",0,null))
r.r.F(0,s)
x=7
return A.c(J.aO(r),$async$m0)
case 7:q=e
x=8
return A.c(B.E.ag(q.w),$async$m0)
case 8:p=e
if(q.b===200){o=B.k.C(0,p,null)
n=A.b([],y.h)
for(k=J.aZ(J.A(o,"data"));k.N();){m=k.ga1(k)
J.cb(n,J.A(m,"City"))}v=new A.x(n,1,null,null,y.m)
x=1
break}else{k=A.X(B.k.C(0,p,null),y.b)
v=new A.x(null,0,k.d,null,y.m)
x=1
break}u=2
x=6
break
case 4:u=3
i=t
l=A.V(i)
k=J.u(l)
v=new A.x(null,0,k,null,y.m)
x=1
break
x=6
break
case 3:x=2
break
case 6:case 1:return A.f(v,w)
case 2:return A.e(t,w)}})
return A.h($async$m0,w)}}
J=c[1]
A=c[0]
B=c[2]
C=a.updateHolder(c[316],C)
var z=a.updateTypes([])
var y={m:A.I("x<q<d>>"),h:A.I("n<d>"),g:A.I("d"),b:A.I("@")}};
((a,b)=>{a[b]=a.current
a.eventLog.push({p:"main.dart.js_87",e:"endPart",h:b})})($__dart_deferred_initializers__,"GndhC5HQJE+pAyaY0ClK7+Zahy8=");